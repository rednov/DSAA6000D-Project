...
def train_ops(model: kgc, learning_rate=0.1, optimizer_str='gradient', regularizer_weight=1.0):
    with tf.device('/cpu'):
...
def main(_):
    parser = argparse.ArgumentParser(description='kgc.')
    ...
    model = kgc(args.data_dir, embed_dim=args.dim, combination_method=args.combination_method,
            ...
            if n_iter % args.save_per == 0 or n_iter == args.max_iter - 1:
                save_path = saver.save(session,
                                       os.path.join(args.save_dir,
                                                    "kgc_" + str(args.prefix) + "_" + str(n_iter) + ".ckpt"))
